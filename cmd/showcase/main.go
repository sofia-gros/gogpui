...
func main() {
	...
	navTreeState := tree.NewTreeState().Items(
		tree.NewItem("forms", "Forms").Expanded(true).Children(
		...
		tree.NewItem("display", "Data Display").Expanded(true).Children(
			tree.NewItem("lbl", "Label"),
			tree.NewItem("bdg", "Badge"),
			tree.NewItem("avt", "Avatar"),
			tree.NewItem("tree", "Tree"),
		),
		tree.NewItem("feedback", "Feedback").Expanded(true).Children(
			tree.NewItem("prog", "Progress"),
			tree.NewItem("skel", "Skeleton"),
		),
		tree.NewItem("nav", "Navigation").Expanded(true).Children(
			tree.NewItem("brd", "Breadcrumb"),
			tree.NewItem("col", "Collapsible"),
		),
		tree.NewItem("layout", "Layout").Expanded(true).Children(
