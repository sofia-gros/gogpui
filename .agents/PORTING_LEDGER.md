...
## 使い方
1. 最初のExploreタスクとして、Antigravityに `crates/ui/src`(gpui-componentのRustソース)配下の
   モジュール一覧を出力させ、下表の行を自動生成させる。手作業で覚えているコンポーネント名を
   使って埋めない(漏れ・誤記の元)。
2. 1コンポーネントの作業が完了するたびに、この台帳を必ず更新する。更新しないままタスク完了を
   報告することを禁止する(AGENTS.md参照)。
...
- Select / Popover / Tooltip などオーバーレイ系は、AGENTS.mdのルール4(毎フレーム座標再計算)を
  明示的に確認しながら進める。
